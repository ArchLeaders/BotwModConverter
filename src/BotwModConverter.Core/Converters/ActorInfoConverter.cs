...
[MatchesName("ActorInfo.product.byml")]
public sealed class ActorInfoConverter : IConverter
{
    private static readonly FrozenDictionary<string, double> _instSizeRatios = new Dictionary<string, double> {
        { "ActorOption", 1.47986 },
    ...
    public SpanOwner<byte> ToSwitch(ArraySegment<byte> data, ModContext context)
    {
        ...
        foreach (var actor in actors) {
            if (!actor.TryGetValue("profile", out var profile)) {
                continue;
            }
